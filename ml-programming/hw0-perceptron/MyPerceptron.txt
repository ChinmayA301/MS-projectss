...
def updateW(w, xt, yt):
    ...
    # placehold to store the updated w, can ignore this line
    # Implement the If statement here based on Line 4-5 of the pseudo-code
    # fill in the code here
    new_w = w.copy()  # Create a copy of the current weight vector
    if yt * np.dot(w, xt) <= 0:
        new_w += yt * xt
        return new_w, True  # Weight updated
    else:
        return new_w, False  # No update
...
def predict(w, X):
    # placehold to store predict result y_hat, can ignore this line
...
def computeError(y, y_hat):
    # placehold to store the error rate w, can ignore this line
